...
void CMycomm::ResetSerial()
{
	...
	//컴포트의 버퍼크기를 설정해준다. 파라미터 1 : 디바이스 파일 핸들러 | 파라미터 2 : 수신 버퍼 | 파라미터 3 : 송신 버퍼
...
UINT CommThread(LPVOID lpData)
{
	...
	while (Comm->m_bIsOpenned)
	{
		...
		//원래 COM 포트에서 발생하는 에러를 검출하는 함수이다. 
		//파라미터 1 : 디바이스 파일 핸들러 | 파라미터 2 : 검출된 에러가 저장됨 | 파라미터 3 : 이 인자를 통해 수신한 데이터 크기를 알 수 있다. COMSTAT.cbInQue 값
		ClearCommError(Comm->m_hComDev, &ErrorFlags, &ComStat);

		if ((EvtMask & EV_RXCHAR) && ComStat.cbInQue)
		{			
			if (ComStat.cbInQue > MAXBUF)
				size = MAXBUF;	//데이터 크기가 MAXBUF보다 크면 MAXBUF로
			else
				size = ComStat.cbInQue; //데이터 크기가 MAXBUF보다 작으면 버퍼사이즈로
			do
			{
				ClearCommError(Comm->m_hComDev, &ErrorFlags, &ComStat);
...
BOOL CMycomm::Send(LPCTSTR outbuf, int len)
{
	...
	//int nLen = WideCharToMultiByte(CP_UTF8, 0, outbuf, -1, sTime, 0, NULL, NULL);
	//sTime = new char[nLen + 1];
	//WideCharToMultiByte(CP_UTF8, 0, outbuf, -1, sTime, 128, NULL, NULL);
...
//데이터 받는곳
int CMycomm::Receive(byte* inbuf, int len)
{
